...
class AbstractApiTransport extends abstract_1.AbstractTransport {
    ...
    listen() {
        ...
        this.sessionsClient.on('releaseRequest', descriptor => {
            this.emit(constants_1.TRANSPORT.DEVICE_REQUEST_RELEASE, descriptor);
        });
    ...
    call({ session, name, data, protocol: customProtocol, signal, }) {
        return this.scheduleAction(async (signal) => {
            ...
            const readResult = await (0, receive_1.receiveAndParse)(this.messages, () => this.api.read(path, signal), protocol);
    ...
    receive({ session, protocol: customProtocol, signal, }) {
        return this.scheduleAction(async (signal) => {
            ...
            const { path } = getPathBySessionResponse.payload;
            const protocol = customProtocol || protocol_1.v1;
            const message = await (0, receive_1.receiveAndParse)(this.messages, () => this.api.read(path, signal), protocol);
            if (!message.success) {
