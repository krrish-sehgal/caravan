// src/wallet.ts
import { bitcoinsToSatoshis as bitcoinsToSatoshis2 } from "@caravan/bitcoin";
...
async function bitcoindListUnspent({
  ...
  try {
    ...
    return resp.result.map((utxo, mapindex) => {
      const amount = new BigNumber2(utxo.amount);
      return {
        confirmed: (utxo.confirmations || 0) > 0,
        txid: utxo.txid,
        index: utxo.vout,
        amount: amount.toFixed(8),
        amountSats: bitcoinsToSatoshis2(amount.toString()),
...
function normalizeTransactionData(txData, clientType) {
  ...
  return {
    ...
    vout: txData.vout.map((output) => ({
      value: clientType === "private" /* PRIVATE */ ? output.value : satoshisToBitcoins(output.value),
...
var BlockchainClient = class extends ClientBase {
  ...
  constructor({
    ...
    if (type !== "private" /* PRIVATE */ && network !== Network.MAINNET && network !== Network.TESTNET && network !== Network.SIGNET) {
      throw new Error("Invalid network");
    }
    if (type !== "mempool" /* MEMPOOL */ && network === Network.SIGNET) {
      throw new Error("Invalid network");
    }
    let host = "";
    ...
    if (type !== "private" /* PRIVATE */ && network !== Network.MAINNET) {
      host += `/${network}`;
    }
  ...
  async formatUtxo(utxo) {
    ...
    const amount = new BigNumber3(utxo.value);
    return {
