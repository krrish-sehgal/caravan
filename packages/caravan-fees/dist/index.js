...
__export(src_exports, {
  ABSURDLY_HIGH_ABS_FEE: () => ABSURDLY_HIGH_ABS_FEE,
  ABSURDLY_HIGH_FEE_RATE: () => ABSURDLY_HIGH_FEE_RATE,
  BtcTransactionTemplate: () => BtcTransactionTemplate,
  BtcTxComponent: () => BtcTxComponent,
  BtcTxInputTemplate: () => BtcTxInputTemplate,
  BtcTxOutputTemplate: () => BtcTxOutputTemplate,
  DEFAULT_DUST_THRESHOLD_IN_SATS: () => DEFAULT_DUST_THRESHOLD_IN_SATS,
  FeeBumpStrategy: () => FeeBumpStrategy,
  RBF_SEQUENCE: () => RBF_SEQUENCE,
  SCRIPT_TYPES: () => SCRIPT_TYPES,
  TransactionAnalyzer: () => TransactionAnalyzer,
  calculateTotalInputValue: () => calculateTotalInputValue,
  calculateTotalOutputValue: () => calculateTotalOutputValue,
  createAcceleratedRbfTransaction: () => createAcceleratedRbfTransaction,
  createCPFPTransaction: () => createCPFPTransaction,
  createCancelRbfTransaction: () => createCancelRbfTransaction,
  createOutputScript: () => createOutputScript,
  estimateTransactionVsize: () => estimateTransactionVsize,
  getOutputAddress: () => getOutputAddress,
  initializePsbt: () => initializePsbt,
  isCPFPFeeSatisfied: () => isCPFPFeeSatisfied,
  mapCaravanNetworkToBitcoinJS: () => mapCaravanNetworkToBitcoinJS,
  parseNonWitnessUtxoValue: () => parseNonWitnessUtxoValue,
  parseWitnessUtxoValue: () => parseWitnessUtxoValue,
  validateCPFPPackage: () => validateCPFPPackage,
  validateNonWitnessUtxo: () => validateNonWitnessUtxo,
...
var SCRIPT_TYPES = {
  ...
  UNKNOWN: "UNKNOWN",
  /** Pay to Script Hash */
  P2SH: import_bitcoin.MULTISIG_ADDRESS_TYPES.P2SH,
  /** Pay to Script Hash wrapping a Pay to Witness Script Hash */
  P2SH_P2WSH: import_bitcoin.MULTISIG_ADDRESS_TYPES.P2SH_P2WSH,
  /** Pay to Witness Script Hash (Native SegWit for scripts) */
  P2WSH: import_bitcoin.MULTISIG_ADDRESS_TYPES.P2WSH
};

// src/utils.ts
var import_bitcoin2 = require("@caravan/bitcoin");
var import_bitcoinjs_lib_v6 = require("bitcoinjs-lib-v6");
var import_psbt = require("@caravan/psbt");
var import_bignumber = __toESM(require("bignumber.js"));
function createOutputScript(destinationAddress, network) {
  const addressValidationError = (0, import_bitcoin2.validateAddress)(destinationAddress, network);
  if (addressValidationError) {
    throw new Error(addressValidationError);
  }
  const bitcoinJsNetwork = network === import_bitcoin2.Network.TESTNET ? import_bitcoinjs_lib_v6.networks.testnet : import_bitcoinjs_lib_v6.networks.bitcoin;
  try {
    return import_bitcoinjs_lib_v6.address.toOutputScript(destinationAddress, bitcoinJsNetwork);
  } catch (error) {
    try {
      const p2wpkh = import_bitcoinjs_lib_v6.payments.p2wpkh({
      ...
      const p2wsh = import_bitcoinjs_lib_v6.payments.p2wsh({
        address: destinationAddress,
        network: bitcoinJsNetwork
      });
      if (p2wsh.output) {
      ...
      const p2tr = import_bitcoinjs_lib_v6.payments.p2tr({
        address: destinationAddress,
        network: bitcoinJsNetwork
      });
      if (p2tr.output) {
...
function getOutputAddress(script, network) {
  ...
  try {
    if (script.length === 25 && script[0] === 118 && script[1] === 169) {
      const p2pkhAddress = import_bitcoinjs_lib_v6.payments.p2pkh({
        output: script,
        network: bitcoinjsNetwork
      }).address;
      if (p2pkhAddress)
    ...
    if (script.length === 22 && script[0] === 0 && script[1] === 20) {
      const p2wpkhAddress = import_bitcoinjs_lib_v6.payments.p2wpkh({
        output: script,
        network: bitcoinjsNetwork
      }).address;
      if (p2wpkhAddress)
    ...
    if (script.length === 34 && script[0] === 0 && script[1] === 32) {
      const p2wshAddress = import_bitcoinjs_lib_v6.payments.p2wsh({
        output: script,
        network: bitcoinjsNetwork
      }).address;
      if (p2wshAddress)
    ...
    if (script.length === 23 && script[0] === 169 && script[22] === 135) {
      return import_bitcoinjs_lib_v6.payments.p2sh({ output: script, network: bitcoinjsNetwork }).address || "";
    }
    if (script.length === 34 && script[0] === 81) {
      const p2trAddress = import_bitcoinjs_lib_v6.payments.p2tr({
        output: script,
        network: bitcoinjsNetwork
      }).address;
      if (p2trAddress)
...
function estimateTransactionVsize({
  ...
  switch (addressType) {
    case SCRIPT_TYPES.P2SH:
      return (0, import_bitcoin2.estimateMultisigP2SHTransactionVSize)({
        numInputs,
        numOutputs,
        m,
        n
    ...
    case SCRIPT_TYPES.P2SH_P2WSH:
      return (0, import_bitcoin2.estimateMultisigP2SH_P2WSHTransactionVSize)({
        numInputs,
        numOutputs,
        m,
        n
    ...
    case SCRIPT_TYPES.P2WSH:
      return (0, import_bitcoin2.estimateMultisigP2WSHTransactionVSize)({
        numInputs,
        numOutputs,
        m,
        n
...
function initializePsbt(psbt) {
  if (psbt instanceof import_psbt.PsbtV2) {
    return psbt;
  }
  try {
    return new import_psbt.PsbtV2(psbt);
  } catch (error) {
    try {
      return import_psbt.PsbtV2.FromV0(psbt);
...
function calculateTotalInputValue(psbt) {
  let total = new import_bignumber.default(0);
...
function parseWitnessUtxoValue(utxo, index) {
  if (!utxo) {
    ...
    return new import_bignumber.default(0);
  }
  try {
    const buffer = Buffer2.from(utxo, "hex");
    const value = buffer.readBigUInt64LE(0);
    return new import_bignumber.default(value.toString());
  } catch (error) {
...
function parseNonWitnessUtxoValue(rawTx, outputIndex) {
  try {
    const tx = import_bitcoinjs_lib_v6.Transaction.fromHex(rawTx);
    ...
    return new import_bignumber.default(output.value);
  } catch (error) {
    throw new Error(
...
function calculateTotalOutputValue(psbt) {
  ...
  return new import_bignumber.default(sum.toString());
}
function mapCaravanNetworkToBitcoinJS(network) {
  switch (network) {
    case import_bitcoin2.Network.MAINNET:
      return import_bitcoinjs_lib_v6.networks.bitcoin;
    case import_bitcoin2.Network.TESTNET:
      return import_bitcoinjs_lib_v6.networks.testnet;
    case import_bitcoin2.Network.REGTEST:
      return import_bitcoinjs_lib_v6.networks.regtest;
    case import_bitcoin2.Network.SIGNET:
      throw new Error(
...
function validateNonWitnessUtxo(utxoBuffer, txid, vout) {
  try {
    const tx = import_bitcoinjs_lib_v6.Transaction.fromBuffer(utxoBuffer);
    ...
    if (!Buffer2.isBuffer(output.script)) {
      return false;
    }
    return true;
  } catch (error) {
...
var BtcTxComponent = class {
  ...
  constructor(amountSats) {
    this._amountSats = new import_bignumber2.default(amountSats);
  }
  /**
  ...
  set amountSats(value) {
    this._amountSats = new import_bignumber2.default(value);
  }
  /**
   ...
   */
  get amountBTC() {
    return (0, import_bitcoin3.satoshisToBitcoins)(this._amountSats.toString());
  }
  hasAmount() {
...
var BtcTxInputTemplate = class _BtcTxInputTemplate extends BtcTxComponent {
  ...
  static fromUTXO(utxo) {
    ...
    if (utxo.prevTxHex) {
      template.setNonWitnessUtxo(Buffer2.from(utxo.prevTxHex, "hex"));
    }
    if (utxo.witnessUtxo) {
...
var BtcTxOutputTemplate = class extends BtcTxComponent {
  ...
  addAmount(amountSats) {
    ...
    this._amountSats = this._amountSats.plus(new import_bignumber2.default(amountSats));
  }
  /**
  ...
  subtractAmount(amountSats) {
    ...
    const subtractAmount = new import_bignumber2.default(amountSats);
...
var TransactionAnalyzer = class _TransactionAnalyzer {
   ...
   */
  get cpfpFeeRate() {
    const desiredPackageFee = new import_bignumber3.default(this.targetFeeRate).multipliedBy(
      this.CPFPPackageSize
    );
    const expectedFeeRate = import_bignumber3.default.max(
   ...
   */
  get minimumRBFFee() {
    return new import_bignumber3.default(this.fee).plus(this._incrementalRelayFeeRate.multipliedBy(this.vsize)).integerValue(import_bignumber3.default.ROUND_CEIL).toString();
  }
  /**
   ...
   */
  get minimumCPFPFee() {
    return new import_bignumber3.default(this.cpfpFeeRate).multipliedBy(this.CPFPPackageSize).toString();
  }
  /**
  ...
  deserializeInputs() {
    return this._rawTx.ins.map((input) => {
      const template = new BtcTxInputTemplate({
        txid: Buffer2.from(input.hash).reverse().toString("hex"),
   ...
   */
  recommendStrategy() {
    if (new import_bignumber3.default(this.feeRate).isGreaterThanOrEqualTo(this.targetFeeRate)) {
      return "NONE" /* NONE */;
    }
    const rbfFee = this.minimumRBFFee;
    const cpfpFee = this.minimumCPFPFee;
    if (this.canRBF && (!this.canCPFP || new import_bignumber3.default(rbfFee).isLessThan(cpfpFee))) {
      return "RBF" /* RBF */;
  ...
  static validateOptions(options) {
    ...
    try {
      const tx = import_bitcoinjs_lib_v62.Transaction.fromHex(options.txHex);
      if (tx.outs.length === 0) {
    ...
    validatedOptions.rawTx = import_bitcoinjs_lib_v62.Transaction.fromHex(options.txHex);
    if (!Object.values(import_bitcoin4.Network).includes(options.network)) {
    ...
    const absoluteFee = new import_bignumber3.default(options.absoluteFee);
    ...
    const incrementalRelayFee = new import_bignumber3.default(
...
var import_psbt2 = require("@caravan/psbt");
...
var BtcTransactionTemplate = class _BtcTransactionTemplate {
  ...
  constructor(options) {
    ...
    this._targetFeeRate = new import_bignumber4.default(options.targetFeeRate);
    this._dustThreshold = new import_bignumber4.default(
  ...
  get needsChange() {
    ...
    const fee = new import_bignumber4.default(this.targetFeesToPay);
  ...
  get feeRateSatisfied() {
    return new import_bignumber4.default(this.estimatedFeeRate).gte(this._targetFeeRate);
  }
  /**
  ...
  get needsChangeOutput() {
    ...
    const changeOutputFee = new import_bignumber4.default(this._targetFeeRate).multipliedBy(
      MAX_OUTPUT_SIZE
    );
  ...
  adjustChangeOutput() {
    ...
    const currentChangeAmount = new import_bignumber4.default(changeOutput.amountSats);
    ...
    if (new import_bignumber4.default(changeOutput.amountSats).lt(this._dustThreshold)) {
  ...
  validate() {
    ...
    const feeRate = new import_bignumber4.default(this.estimatedFeeRate);
    if (feeRate.gte(new import_bignumber4.default(ABSURDLY_HIGH_FEE_RATE))) {
      return false;
    }
    if (this.calculateCurrentFee().gte(new import_bignumber4.default(ABSURDLY_HIGH_ABS_FEE))) {
      return false;
    }
    return true;
  }
  ...
  toPsbt(validated = true) {
    ...
    const psbt = new import_psbt2.PsbtV2();
  ...
  calculateTotalInputAmount() {
    return this.inputs.reduce((sum, input) => {
      ...
      return sum.plus(new import_bignumber4.default(input.amountSats));
    }, new import_bignumber4.default(0));
  }
  /**
   * Calculates the total output amount.
  ...
  calculateTotalOutputAmount() {
    return this.outputs.reduce((sum, output) => {
      ...
      return sum.plus(new import_bignumber4.default(output.amountSats));
    }, new import_bignumber4.default(0));
  }
  /**
  ...
  calculateChangeAmount() {
    return this.outputs.reduce(
      (acc, output) => output.isMalleable ? acc.plus(new import_bignumber4.default(output.amountSats)) : acc,
      new import_bignumber4.default(0)
    );
  }
  /**
   ...
   */
  targetFees() {
    return this._targetFeeRate.times(this.calculateEstimatedVsize()).integerValue(import_bignumber4.default.ROUND_CEIL);
  }
  /**
  ...
  static processInputs(psbt) {
    ...
    for (let i = 0; i < psbt.PSBT_GLOBAL_INPUT_COUNT; i++) {
      ...
      const input = new BtcTxInputTemplate({
        txid: Buffer2.from(txid, "hex").reverse().toString("hex"),
        vout
      });
  ...
  static setInputUtxo(input, psbt, index) {
    ...
    if (witnessUtxo) {
      ...
      const witnessUtxoBuffer = Buffer2.from(witnessUtxo, "hex");
      ...
      input.setNonWitnessUtxo(Buffer2.from(nonWitnessUtxo, "hex"));
      input.amountSats = amountSats;
      input.setNonWitnessUtxo(Buffer2.from(nonWitnessUtxo, "hex"));
      input.amountSats = amountSats;
    } else {
  ...
  static processOutputs(psbt, network) {
    ...
    for (let i = 0; i < psbt.PSBT_GLOBAL_OUTPUT_COUNT; i++) {
      const script = Buffer2.from(psbt.PSBT_OUT_SCRIPT[i], "hex");
...
var validateRbfPossibility = (txAnalyzer, fullRBF, strict) => {
  ...
  if (new import_bignumber5.default(txAnalyzer.targetFeeRate).isLessThanOrEqualTo(
    txAnalyzer.feeRate
  )) {
    throw new Error(
...
var addAdditionalInputs = (newTxTemplate, availableInputs, addedInputs, txAnalyzer) => {
  for (const utxo of availableInputs) {
    if (newTxTemplate.feeRateSatisfied && new import_bignumber5.default(newTxTemplate.currentFee).gte(txAnalyzer.minimumRBFFee) && (newTxTemplate.needsChange || newTxTemplate.outputs.length > 0)) {
      break;
    }
...
var validateRbfRequirements = (newTxTemplate, txAnalyzer) => {
  const currentFee = new import_bignumber5.default(newTxTemplate.currentFee);
  const minRequiredFee = new import_bignumber5.default(txAnalyzer.minimumRBFFee);
  const targetFee = new import_bignumber5.default(newTxTemplate.targetFeesToPay);
...
var createCancelRbfTransaction = (options) => {
  ...
  if (new import_bignumber5.default(targetFeeRate).isLessThanOrEqualTo(txAnalyzer.feeRate)) {
    throw new Error(
  ...
  const totalInputAmount = new import_bignumber5.default(newTxTemplate.totalInputAmount);
  const totalOutputAmount = new import_bignumber5.default(newTxTemplate.totalOutputAmount);
  const fee = import_bignumber5.default.max(
...
var createAcceleratedRbfTransaction = (options) => {
  ...
  const newTxTemplate = new BtcTransactionTemplate({
    inputs: [],
    outputs: [],
    targetFeeRate: import_bignumber5.default.max(
      options.targetFeeRate,
      new import_bignumber5.default(txAnalyzer.minimumRBFFee).dividedBy(txAnalyzer.vsize)
    ).toNumber(),
    dustThreshold: options.dustThreshold,
    network: options.network,
    scriptType: options.scriptType,
...
function isCPFPFeeSatisfied(txAnalyzer, childTxTemplate) {
  ...
  const parentFee = new import_bignumber6.default(txAnalyzer.fee);
  const parentVsize = txAnalyzer.vsize;
  const childFee = new import_bignumber6.default(childTxTemplate.currentFee);
...
function validateCPFPPackage(txAnalyzer, childTxTemplate, strict) {
  const parentFee = new import_bignumber6.default(txAnalyzer.fee);
  const parentSize = new import_bignumber6.default(txAnalyzer.vsize);
  const childFee = new import_bignumber6.default(childTxTemplate.currentFee);
  const childSize = new import_bignumber6.default(childTxTemplate.estimatedVsize);
